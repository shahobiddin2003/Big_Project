...
let resArr = products.filter((el) => el.discount >= 50).slice(0, 4);
let cardsBox = document.querySelector(".price_off_cards");
let showCards = resArr.map((el, i) => {
  return `
  <div class="card  rounded-2">
              <div class="card_image position-relative">
                <img
                  src="${el.images[0]}"
                  alt=""
                  class="card_image_main mx-auto p-1 rounded-2"
                />
                <img
                  src="./assets/images/Heart.png"
                  alt=""
                  class="like position-absolute top-0 end-0 p-2"
                />
                <div
                  class="discount_percent position-absolute bottom-0 start-0 translate-middle-y translate-middle-x"
                >
                  <p class="percent p-1">${el.discount} %</p>
                </div>
              </div>
              <div class="card_price_off_content p-2">
                <div class="card_discount d-flex justify-content-between">
                  <p class="card_discount_percent_creditcard">${Math.floor(
                    el.price * 0.88
                  )} P</p>
                  <p class="card_discount_percent_cash">${el.price} P</p>
                </div>
                <div class="card_price_way d-flex justify-content-between">
                  <p class="credit_card">С картой</p>
                  <p class="cash">Обычная</p>
                </div>
                <p class="product_description">
                  ${el.description}
                </p>
                <div class="star_rating">
                  <span class="star_rating fa fa-star checked"></span>
                  <span class="star_rating fa fa-star checked"></span>
                  <span class="star_rating fa fa-star checked"></span>
                  <span class="star_rating fa fa-star checked"></span>
                  <span class="star_rating fa fa-star"></span>
                </div>
                <button class="card_button rounded-1">В корзину</button>
              </div>
            </div>
  `;
});

cardsBox.innerHTML = showCards;

let newProducts = products.slice(0, 4);
let newProductsCardBox = document.querySelector(".new_products_cards");
let showNewProductsCards = newProducts.map((el, i) => {
  return `
  <div class="card  rounded-2">
              <div class="card_image position-relative">
                <img
...
newProductsCardBox.innerHTML = showNewProductsCards;

// ///////////////////////////////

let lastBuyings = products.slice(products.length - 4, products.length);
let lastBuyingsCardBox = document.querySelector(".last_buyings_cards");
let showLastBuyingsCards = lastBuyings.map((el, i) => {
  return `
  <div class="card  rounded-2">
              <div class="card_image position-relative">
                <img
...
lastBuyingsCardBox.innerHTML = showLastBuyingsCards;
